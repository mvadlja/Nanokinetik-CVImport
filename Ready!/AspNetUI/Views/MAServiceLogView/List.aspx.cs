...
namespace AspNetUI.Views.MAServiceLogView
{
    public partial class List : ListPage
    {
        ...
        private IMa_service_log_PKOperations _maServiceLogOperations;

        #endregion

        #region Properties

        private int? _maServiceLogPkToDelete
        {
            get { return (int?)ViewState["_maServiceLogPkToDelete"]; }
            set { ViewState["_maServiceLogPkToDelete"] = value; }
        }
        ...
        private void BindGrid()
        {
            ...
            if (gobList.Count == 0) gobList.Add(new GEMOrderBy("ma_service_log_PK", GEMOrderByType.DESC));
            ...
            {
                ds = _maServiceLogOperations.GetListFormDataSet(filters, ServiceLogGrid.CurrentPage, ServiceLogGrid.PageSize, gobList, out itemCount);
            }
        ...
        private void DeleteEntity(int? entityPk)
        {
            ...
            {
                _maServiceLogOperations.Delete(entityPk);
                Response.Redirect("~/Views/ServiceLogView/List.aspx");
            }
        ...
        public void btnDeleteEntity_OnClick(object sender, EventArgs e)
        {
            ...
            {
                _maServiceLogPkToDelete = ValidationHelper.IsValidInt(commandArgumentString) ? int.Parse(commandArgumentString) : (int?)null;
                if (_maServiceLogPkToDelete.HasValue) mpDelete.ShowModalPopup("Warning!", "<center>Are you sure that you want to delete this record?</center><br />", ModalPopupMode.YesNo);
            }
        }

        private void mpDelete_OnYesButtonClick(object sender, EventArgs e)
        {
            DeleteEntity(_maServiceLogPkToDelete);
        }
        ...
        void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (ListType == ListType.List)
            {
                ds = _maServiceLogOperations.GetListFormDataSet(filters, 1, Int32.MaxValue, gobList, out itemCount);
            }

            ServiceLogGrid["ma_service_log_PK"].Visible = true;
            if (ds != null) ServiceLogGrid.ExportDataToXlsx(PrepareDataForExport(ds.Tables[0]), new PossGrid.ExcellExportOptions("grid"));
            ServiceLogGrid["ma_service_log_PK"].Visible = false;
        ...
        private void GenerateTabMenuItems()
        {
            ...
            if (ListType == ListType.List)
            {
                location =  Support.LocationManager.Instance.GetLocationByName("Level3-MAServiceStats", Support.CacheManager.Instance.AppLocations);
