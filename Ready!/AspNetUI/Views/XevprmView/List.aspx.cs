...
namespace AspNetUI.Views.XevprmView
{
    public partial class List : ListPage
    {
        ...
        public override void LoadFormVariables()
        {
            ...
            {
                XevprmGrid.GridVersion = XevprmGrid.GridVersion + ListType.ToString();
            }

            _gridId = XevprmGrid.GridId + "_" + XevprmGrid.GridVersion;
        }
        ...
        private void BindEventHandlers()
        {
            ...
            XevprmGrid.OnRebindRequired += XevprmGridOnRebindRequired;
            XevprmGrid.OnHtmlRowPrepared += XevprmGridOnHtmlRowPrepared;
            XevprmGrid.OnHtmlCellPrepared += XevprmGridOnHtmlCellPrepared;
            XevprmGrid.OnExcelCellPrepared += XevprmGridOnExcelCellPrepared;
            XevprmGrid.OnLoadClientLayout += XevprmGridOnLoadClientLayout;
        ...
        void FillXevprmStatus(object arg)
        {
            ...
            XevprmGrid.SetComboList("XevprmStatus", xevprmStatusListItems);
        }
        ...
        void FillOperation(object arg)
        {
            ...
            XevprmGrid.SetComboList("Operation", operationListItems);
        }
        ...
        void FillSubmissionStatus(object arg)
        {
            ...
            XevprmGrid.SetComboList("GatewaySubmissionStatus", submissionStatusListItems);
        }
        ...
        void BindForm(object arg)
        {
            ...
            {
                XevprmGrid.SetClientLayoutBeforeBind(userGridSettings.grid_layout);
            }
        }
        ...
        private void BindGrid()
        {
            ...
            if (XevprmGrid.SecondSortingColumn != null) gobList.Add(new GEMOrderBy(XevprmGrid.SecondSortingColumn, XevprmGrid.SecondSortingOrder == PossGrid.SortOrder.ASC ? GEMOrderByType.ASC : GEMOrderByType.DESC));

            if (XevprmGrid.MainSortingColumn != null) gobList.Add(new GEMOrderBy(XevprmGrid.MainSortingColumn, XevprmGrid.MainSortingOrder == PossGrid.SortOrder.ASC ? GEMOrderByType.ASC : GEMOrderByType.DESC));
            if (gobList.Count == 0) gobList.Add(new GEMOrderBy("xevprm_message_PK", GEMOrderByType.DESC));

            var itemCount = 0;

            var ds = _xevprmMessageOperations.GetListFormDataSet(filters, XevprmGrid.CurrentPage, XevprmGrid.PageSize, gobList, out itemCount);

            XevprmGrid.TotalRecords = itemCount;

            if (XevprmGrid.CurrentPage > XevprmGrid.TotalPages || (XevprmGrid.CurrentPage == 0 && XevprmGrid.TotalPages > 0))
            {
                if (XevprmGrid.CurrentPage > XevprmGrid.TotalPages) XevprmGrid.CurrentPage = XevprmGrid.TotalPages; else XevprmGrid.CurrentPage = 1;

                ds = _xevprmMessageOperations.GetListFormDataSet(filters, XevprmGrid.CurrentPage, XevprmGrid.PageSize, gobList, out itemCount);
            }

            XevprmGrid.DataSource = ds != null ? ds.Tables[0].DefaultView : null;
            XevprmGrid.DataBind();
        ...
        private void DeleteEntity(int? entityPk)
        {
            if (entityPk.HasValue)
            {
                ...
                Response.Redirect(string.Format("~/Views/XevprmView/List.aspx?EntityContext={0}", EntityContext.Xevprm));
            }
        ...
        public void OnContextMenuItemClick(object sender, ContextMenuEventArgs e)
        {
            switch (e.EventType)
            {
                case ContextMenuEventTypes.Back:
                    {
                        if (EntityContext == EntityContext.AuthorisedProduct) Response.Redirect(string.Format("~/Views/AuthorisedProductView/List.aspx?EntityContext={0}", EntityContext.AuthorisedProduct));
        ...
        void XevprmGridOnExcelCellPrepared(object sender, PossGrid.PossGridExportCellRenderArgs args)
        {
        ...
        void btnColumns_OnClick(object sender, EventArgs e)
        {
            ...
            foreach (DataControlField column in XevprmGrid.Columns)
            {
                if (column is IFilteredColumn && (column as IFilteredColumn).FieldName.NotIn("xevprm_message_PK", "XevprmXmlPdf", "XevprmXmlRtf", "AckXmlPdf", "AckXmlRtf", "SubmittedBy"))
                {
                    ...
                    if (XevprmGrid.VisibleColumns.Contains((column as IFilteredColumn).FieldName))
                    {
        ...
        {
            XevprmGrid.VisibleColumns = ColumnsPopup.SelectedColumns.Cast<ListItem>().Select(x => x.Value).ToList();
        ...
        void btnSaveLayout_Click(object sender, EventArgs e)
        {
            ...
            var userGridSettings = new User_grid_settings_PK
            {
                ...
                grid_layout = XevprmGrid.GetClientLayoutString(),
        ...
        void btnExport_Click(object sender, EventArgs e)
        {
            ...
            XevprmGrid["XevprmXml"].Visible = true;
            XevprmGrid["XevprmXml"].Caption = "MSG XML";
            XevprmGrid["XevprmXmlPdf"].Visible = true;
            XevprmGrid["XevprmXmlRtf"].Visible = true;
            XevprmGrid["AckXml"].Visible = true;
            XevprmGrid["AckXml"].Caption = "ACK XML";
            XevprmGrid["AckXmlPdf"].Visible = true;
            XevprmGrid["AckXmlRtf"].Visible = true;
            XevprmGrid["SubmittedBy"].Visible = true;
            XevprmGrid["Delete"].Visible = false;

            if (ds != null && ds.Tables.Count > 0) XevprmGrid.ExportDataToXlsx(PrepareDataForExport(ds.Tables[0]), new PossGrid.ExcellExportOptions("grid"));

            XevprmGrid["XevprmXml"].Visible = false;
            XevprmGrid["XevprmXml"].Caption = "MSG XML";
            XevprmGrid["XevprmXmlPdf"].Visible = false;
            XevprmGrid["XevprmXmlRtf"].Visible = false;
            XevprmGrid["AckXml"].Visible = false;
            XevprmGrid["AckXml"].Caption = "ACK XML";
            XevprmGrid["AckXmlPdf"].Visible = false;
            XevprmGrid["AckXmlRtf"].Visible = false;
            XevprmGrid["SubmittedBy"].Visible = false;
            XevprmGrid["Delete"].Visible = true;
        }

        private void btnReset_Click(object sender, EventArgs e)
        {
            _userGridSettingsOperations.DeleteLayoutsByUsernameAndGrid(Thread.CurrentPrincipal.Identity.Name, _gridId);
            XevprmGrid.ResetVisibleColumns();
            XevprmGrid.SecondSortingColumn = null;
            
            BindGrid();
        }

        void XevprmGridOnHtmlRowPrepared(object sender, PossGrid.PossGridRowEventArgs e)
        {
        ...
        void XevprmGridOnRebindRequired(PossGrid.PossGrid grid)
        {
            BindGrid();
        }

        void XevprmGridOnHtmlCellPrepared(object sender, PossGrid.PossGridCellEventArgs e)
        {
            if (!XevprmGrid.IsSorted(e.FieldName)) return;

            if (_sortCount == 0)
            {
                _sortCount = XevprmGrid.SortCount;
                _flip = !_flip;
            }

            _sortCount--;

            if (XevprmGrid.SortCount > 1 && e.FieldName == XevprmGrid.MainSortingColumn)
                return;

            e.Cell.CssClass = _flip ? "sorted_column_even" : "sorted_column_odd";
        }

        void XevprmGridOnLoadClientLayout(object sender, PossGrid.ClientLayoutEventArgs args)
        {
        ...
        private void HandleXevprmMessageOperationResultError<T>(OperationResult<T> operationResult, int? xevprmMessagePk = null, XevprmStatus xevprmStatus = XevprmStatus.NULL)
        {
            ...
            string message = string.Format("<div style='text-align:center'>Unexpected error occured!<br/>Please contact your system administrator.<br/><br/>Error ID: {0}<br/><br/></div>", errorId);
        ...
        {
            var filters = XevprmGrid.GetFilters();
        ...
        private void GenerateContextMenuItems()
        {
            ...
            {
                MasterPage.ContextMenu.SetContextMenuItemsVisible(new ContextMenuItem[] { });
            }
        }
