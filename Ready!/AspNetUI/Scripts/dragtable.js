...
// Here's the notice from Mike Hall's draggable scriptgrouppingr
...
dragtable = {
    ...
    init: function (grid) {
        ...
        dragtable.makeDraggable(grid);

        $(grid).colResizable({ disable: true });
        $(grid).colResizable({ liveDrag: true, postbackSafe: true });
    },
    ...
    makeDraggable: function (table) {
        ...
        var headers = $(table).find("tr:eq(1)").children(); // tHead.rows[0].cells;

        for (var i = 0; i < headers.length; i++) {
            if ($(headers[i]).hasClass("fixed-position")) continue;
    ...
    fullCopy: function (elt, deep) {
        ...
        for (ss in elt.style) {
            ...
            if (elt.style.ss == null) continue;
            if (typeof (elt.style.ss) == "string" && elt.style.ss == 0) return;

            new_elt.style[ss] = elt.style[ss];
        }
    ...
    dragStart: function (event, id) {
        ...
        var copySectionColumn = function (sec, col) {
            ...
            $.each(sec.rows, function (key, row) {

                if (!$(row).hasClass("grouppingRow") && !$(row).hasClass("emptyRow")) {
        ...
        $.each(table.tBodies, function (k,tb) {
    ...
    dragEnd: function (event) {
        ...
        if (targetCol != -1 && targetCol != dragObj.startCol && !($(dragObj.table.tHead.rows[0].cells[targetCol]).hasClass("fixed-position"))) {
    ...
    moveColumn: function (table, sIdx, fIdx) {
        ...
        while (i--) {
            row = table.rows[i]
            if ($(row).hasClass("grouppingRow") || $(row).hasClass("emptyRow")) continue;
