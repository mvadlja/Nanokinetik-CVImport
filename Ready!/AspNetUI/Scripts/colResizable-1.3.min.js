...
//        if (t.find("#placeholder-row").length==0){
//            var columnCount= t.find("tr").first().children().length;
//            var placeholderColumn=$("<tr id=\"placeholder-row\"></tr>");
//            for (i =0; i<columnCount; i++) placeholderColumn.prepend("<th id=\""+t.attr("id")+"-pl-"+i+"\"></th>");
//            placeholderColumn.children().css("padding","0px");
//             placeholderColumn.children().css("height","2px");
//            t.children().first().prepend(placeholderColumn);
//        }
    ...
    */
    var createGrips = function (t) {
        ...
        if (t.p && jQuery.cookie("column-sizes-" + t.id) != null) memento(t, th); 	//if 'postbackSafe' is enabled and there is data for the current table, its coloumn layout is restored
        var realIndex=0;
        th.each(function (i) {						//iterate through the table column headers	
            ...
            if (i < t.ln - 1 && !$(this).hasClass("fixed-size") ) g.mousedown(onGripMouseDown).append(t.opt.gripInnerHtml).append('<div class="' + SIGNATURE + '" style="cursor:' + t.opt.hoverCursor + '"></div>'); //bind the mousedown event to start dragging 
    ...
    */
    var memento = function (t, th) {
        ...
        if (th) {										//in deserialization mode (after a postback)
            ...
            if (jQuery.cookie("column-sizes-" + t.id) == null || jQuery.cookie("column-sizes-" + t.id) =="") return;

            w =  jQuery.cookie("column-sizes-" + t.id).split(";");

            for (; i < t.ln; i++) {						//for each column
                var widthParts = w[i].split(("(||)"));

                aux[widthParts[0]] = (100 * (widthParts[1] / (1.0*w[t.ln])) + "%"); 	//width is stored in an array since it will be required again a couple of lines ahead
           
                th.filter("#" + widthParts[0]).first().css("width", aux[widthParts[0]]); 	//each column width in % is resotred
            }
            //  for (i = 0; i < t.ln; i++)
            //        t.cg.eq(i).css("width", aux[i]); //this code is required in order to create an inline CSS rule with higher precedence than an existing CSS class in the "col" elements
        } else {	
        						//in serialization mode (after resizing a column)
            var serialized = ""; 			//clean up previous data
            for (i in t.c) {				//iterate through columns
                w = t.c[i].width(); 	//width is obtained
               
                serialized += (t.c[i].attr("id") + "(||)" + w + ";"); 	//width is appended to the sessionStorage object using ID as key
                m += w; 				//carriage is updated to obtain the full size used by columns
            }
            serialized += m; 				//the last item of the serialized string is the table's active area (width), 

            jQuery.cookie("column-sizes-" + t.id, serialized, { expires: 365, path:"/" });
    
           // $("#pager").text(serialized+$("#pager").text())
    ...
    */
    var onGripDragOver = function (e) {
        ...
        if (t.p) memento(t); 						//if postbackSafe is enabled and there is sessionStorage support, the new layout is serialized and stored
