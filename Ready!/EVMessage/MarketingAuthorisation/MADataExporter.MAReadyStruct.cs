                ...
                foreach (AuthorisedProductStruct authorisedProductStruct in _maReadyStruct.AuthorisedProductStructList)
                {
                    ...
                    if (authorisedProductStruct.ProductStruct.PharmaceuticalProductStructList.Count > 0)
                    {
                        foreach (PharmaceuticalProductStruct pharmaceuticalProductStruct in authorisedProductStruct.ProductStruct.PharmaceuticalProductStructList)
                        {
                            ...
                            //Update Product and Pharmaceutical product calculated columns
                            _pharmaceutical_product_PKOperations.UpdateCalculatedColumn(dbPharmaceuticalProductPK.Value, Pharmaceutical_product_PK.CalculatedColumn.All);
                            _product_PKOperations.UpdateCalculatedColumnByPharmaceuticalProduct(dbPharmaceuticalProductPK.Value, Product_PK.CalculatedColumn.ActiveSubstances);
                        }
                    }
                    ...
                    //Update Product and Pharmaceutical product calculated columns
                    _pharmaceutical_product_PKOperations.UpdateCalculatedColumnByProduct(authorisedProductStruct.ProductStruct.Product.product_PK.Value, Pharmaceutical_product_PK.CalculatedColumn.Products);
                    _product_PKOperations.UpdateCalculatedColumn(authorisedProductStruct.ProductStruct.Product.product_PK.Value, Product_PK.CalculatedColumn.All);
                    ...
                    if (authorisedProductStruct.MeddraList.Count > 0)
                    {
                        for (int meddraIndex = 0; meddraIndex < authorisedProductStruct.MeddraList.Count; meddraIndex++)
                        {
                            try
                            {
                                authorisedProductStruct.MeddraList[meddraIndex].UpdateFullName(_type_PKOperations);
                                ...
                                {
                                    var dbMeddraApMN = new Meddra_ap_mn_PK();
                    ...
                    if (authorisedProductStruct.DocumentStructList.Count > 0)
                    {
                        for (int documentIndex = 0; documentIndex < authorisedProductStruct.DocumentStructList.Count; documentIndex++)
                        {
                            ...
                            //Update Document calculated columns
                            if (dbDocumentPK.HasValue)
                            {
                                _document_PKOperations.UpdateCalculatedColumn(dbDocumentPK.Value, Document_PK.CalculatedColumn.All);    
                            }
                        }
                    }
                }
